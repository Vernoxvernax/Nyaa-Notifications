...
pub struct NyaaTorrent {
  ...
  pub comments: Option<Vec<NyaaComment>>
}
...
impl std::fmt::Display for NyaaTorrent {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(f, " Title: {}\n  Category: {}\n  Comments: {}\n  Size: {}\n  Torrent-file link: {}\n  Magnet-Link: {}\n  Upload-Date: {}\n  Seeders: {}\n  Leechers: {}\n  Completed: {}\n Comments: {:#?}", 
    self.title, self.category, self.comment_amount, self.size, self.torrent_file, self.magnet, self.date, self.seeders, self.leechers, self.completed, self.comments)
  }
}
                ...
                if updates.is_empty() {
                  ...
                  for update in updates.clone() {
                    if update.new_torrent && channel.releases {
                      ...
                      let timestamp = 
                        &update.nyaa_torrent.timestamp.to_string()
                        [0..update.nyaa_torrent.timestamp.to_string().len()].parse::<i64>().unwrap();
                      let nanosec = 
                        &update.nyaa_torrent.timestamp.to_string()
                        [update.nyaa_torrent.timestamp.to_string().len() - 3..update.nyaa_torrent.timestamp.to_string().len()].parse::<u32>().unwrap();
                      let utc_time = 
                        chrono::DateTime::<chrono::Utc>::from_utc(NaiveDateTime::from_timestamp_opt(*timestamp, *nanosec).unwrap(), chrono::Utc);
                    ...
                    if update.new_comments > 0 && channel.comments {
                      ...
                      update.new_comments..update.nyaa_torrent.comments.as_ref().unwrap().len() as u64 {
                        let nyaa_comment = update.nyaa_torrent.comments.as_ref().unwrap().get(comment_index as usize).unwrap();
                        ...
                        let utc_time_comment = chrono::DateTime::<chrono::Utc>::from_utc(
                          NaiveDateTime::from_timestamp_opt(seconds, nanosec).unwrap(), chrono::Utc);
                        if nyaa_comment.user.len() + nyaa_comment.message.len() <= 1024 {
                          ...
                          .send_message(&ctx1, |m| {
                            m.embed(|e| {
                              ...
                              c.create_action_row(|r| {
                                r.create_button(|b| {
                                  b.label("Comment@Nyaa")
                                  .url(
                                    update.nyaa_torrent.torrent_file.replace("download", "view").strip_suffix(".torrent").unwrap().to_owned()+
                                    &nyaa_comment.link.clone()
                                  )
                                  .style(serenity::model::prelude::component::ButtonStyle::Link)
                                })
                                .create_button(|b| {
...
async fn main() {
  ...
  if ! Path::new("./data").exists() {
    ...
    file.write_all(toml::to_string_pretty(&default_config)
    .expect("Failed to create config template.").as_bytes()).expect("Failed writing config template");
  ...
  if config_file.discord_bot.enabled {
    tokio::spawn(async move {
      loop {
        if start_discord_bot(&config_clone).await.is_err() {
  ...
  for page in page_array {
    for mut torrent in page.torrents {
      if ! torrent_file_links.contains(&torrent.torrent_file) {
        let nyaa_comments_res: Result<Vec<NyaaComment>, ()> = if torrent.comment_amount > 0 &&
        ...
        updates.append(&mut [Update {
          ...
          new_comments: torrent.comment_amount,
...
async fn start_discord_bot(config_file: &ConfigFile) -> Result<(), SerenityError> {
    ...
    if config_file.gotfiy.enabled {
      ...
      if update.new_comments > 0 && config_file.gotfiy.comment_notifications {
        ...
        update.new_comments..update.nyaa_torrent.comments.clone().unwrap().len() as u64 {
          ...
          let comment = comments.get(comment_index as usize).unwrap();
          let post_request = json!({
...
fn send_gotify(config_file: &ConfigFile, json: serde_json::Value) -> Result<(), ()> {
  ...
  let message = Request::post(
  config_file.gotfiy.domain.clone() + "/message?token=" + config_file.gotfiy.token.as_str())
